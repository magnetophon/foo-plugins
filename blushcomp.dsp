...
threshold   = compressor_group(hslider("[2] Threshold [unit:dB]   [tooltip: When the signal level exceeds the Threshold (in dB), its level is compressed according to the Ratio]", -30.1, -80, 0, 0.1));
ratio       = compressor_group(hslider("[3] Ratio   [tooltip: A compression Ratio of N means that for each N dB increase in input signal level above Threshold, the output level goes up 1 dB]", 20, 1, 20, 0.1));
...
feedFwBw      = post_group(hslider("[6]feedback/feedforward", 0.008, 0, 1 , 0.001));
outgain       = post_group(hslider("[5]output gain (dB)",           0,      -40,   40,   0.1):smooth(0.999)); // DB

rms_speed        = hslider("RMS speed",200, 1,   440,   1); //0.0005 * min(192000.0, max(22050.0, SR));
