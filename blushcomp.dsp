...
maxGR     = hslider("[5] Max Gain Reduction [unit:dB]   [tooltip: The maximum gain reduction]",0, -140, 0, 0.1) : db2linear : smooth(0.999);
...
prePower = hslider("pre power", 1, 1, 11 , 0.001)*3:pow(3);
postPower = hslider("post power", 1, 1, 11 , 0.001)*3:pow(3);
...
COMP = 1/(((1/((_ <: ( HPF : DETECTOR : RATIO : db2linear :pow(prePower):linear2db<: ( RATELIMITER ~ _ ),_:crossfade(ratelimit) : db2linear )):pow(1/postPower))):max(maxGR)*maxGR*2*PI:tanh:/(2*PI))/maxGR);
